...
class _field {
     ...
     * @return boolean - True si la valeur est acceptée sinon False
     */
    public function setValueForm($value,$arrayPOST){
        ...
        if(!empty($this->get("input"))) {
            ...
            elseif ($this->get("input")["confirmationNeeded"] === true && isset($arrayPOST[$this->get("name")."Confirm"])) {
            ...
            elseif (isset($arrayPOST[$this->get("name")."Hidden"])){
                return $this->setValue($arrayPOST[$this->get("name")."Hidden"]);
            }
            elseif($this->get("input")["type"] === "file") {
    ...
    public function getFormat($format){
        ...
        if($this->type === "datetime" && !empty($this->value)) {
            $objDate = new DateTime($this->value);
            $value = $objDate->format($format);
        }
        else {
            $value = $this->value;
        }
     ...
     *
     * @return mixed Tableau des valeurs du champ sous toutes ses formes ou false
     */
    public function getToArray() {
        ...
        if($this->visibility === true) {
            ...
            $arrayResultat["object"] = (!empty($this->nomObjet)) ? $this->getObjet()->getToArray() : "";
            $arrayResultat["html"] = (!empty($this->formats["html"])) ? $this->getFormat("html") : $this->getValue();
            $arrayResultat["affichage"] = (!empty($this->formats["affichage"])) ? $this->getFormat("affichage") : $this->getValue();
            $arrayResultat["list_libelle"] = (!empty($this->listCleValeur)) ? $this->getListLibelle() : "";
    ...
    /**
     * Retourne la valeur affichable HTML pour l'attribut passé en paramètre
     *
     * @return mixed Libellé affichable de l'attribut  ou false
     */
    public function getAffichageHTML() {
        // On retourne le champ après avoir échappé les caractères HTML et gérer les retour à la ligne
     ...
     */
    public function getElementFormulaire($infosChamp = [], $acces = "", $others = []){
        ...
        if(!empty($this->get("input"))) {
            ...
            switch ($this->get("input")["type"]) {
                ...
                case 'select':
                    ...
                    if(!empty($this->get("nomObjet"))){
                        //On récupère le nom de l'objet
                        $nomObjet = $this->get("nomObjet");
                        // On instancie notre objet
                        $obj = new $nomObjet();
                        // On parcourt les éléments pour créé la liste clé-valeur attendu
                        $listeElement = $obj->list();
                        foreach ($listeElement as $cle => $valeur) {
                            $listOptions[$cle] = $valeur->getValue($obj->abrev_table()."_libelle");
                        }
                    }
                    else {
                        $listOptions = $this->get("listCleValeur");
                    }
                    ...
                    $templateHTML = $divInputGlobal.$labelPrincipal.$inputPrincipal. '<p class="p-error"  id="error-' . $this->get("input")["id"] . '"></p></div>';
                    break;
                ...
                case 'textarea':
                    ...
                    if(isset($this->get("input")["placeholder"])){
                        $inputPrincipal .= 'placeholder="' . $this->get("input")["placeholder"] . '" ';
                    }

                    $inputPrincipal .= $acces . '>' . $this->get("value") . '</textarea>';
                    // On construit le template
                    $templateHTML = $divInputGlobal.'<div>'.$labelPrincipal.$inputPrincipal. '<p class="p-error"  id="error-' . $this->get("input")["id"] . '"></p></div></div>';
                    break;
                //Si on est sur un checkbox
                case 'checkbox':
                    // Si on a un niveau d'accès au champ défini, on le spécifie sur la checkbox (un peu différent)
                    $accesCheckbox = ((!empty($acces)) ? 'disabled' : "");

                    $inputPrincipal = '<input type="checkbox" name="' . $name . '" id="' . $this->get("input")["id"] . '" ';
                    // On met la valeur par défaut de la checkbox (checked ou no)
                    if($this->get("value") == 1) {
                        $inputPrincipal .= 'checked ';
                    }

                    // On construit le template
                    $templateHTML = $divInputGlobal.$labelPrincipal.$inputPrincipal.$accesCheckbox. '><p class="p-error"  id="error-' . $this->get("input")["id"] . '"></p></div>';
                    break;
                ...
                case 'radio':
                    ...
                    $templateHTML .= '<p class="p-error"  id="error-' . $this->get("input")["id"] . '"></p></div>';
                    break;
                case 'hidden':
                ...
                case 'file':
                    ...
                    // Si un fichier est déjà présent et selon son type
                    $valueFile = '';
                    if($this->getValue() != 0) {
                        // On récupère l'objet pièce jointe
                        $objPiece = $this->getObjet();
                        // Si c'est une image, on l'affiche
                        if(substr($objPiece->getValue("pj_type_fichier"),0,5) === "image") {
                            $valueFile = '<div class="div_'.$this->get("input")["id"].'_value">
                                <img src="'.$objPiece->get_url().'" alt="Image actuelle">
                            </div>';
                        }
                        else {
                            //Sinon on affiche un bouton de téléchargement avec un lien vers le fichier
                            $valueFile = '<div class="div_'.$this->get("input")["id"].'_value">
                                <a href="'.$objPiece->get_url().'" title="Téléchargement du fichier actuelle"><img src="public/images/icons/download.png" alt="Icone de téléchargement"></a>
                            </div>';
                        }
                    }
                    
                    $templateHTML = $divInputGlobal.'<div>'.$valueFile.$labelPrincipal.$inputPrincipal.$inputContraintes.$acces.'><p class="p-error"  id="error-' . $this->get("input")["id"] . '"></p></div></div>';
                    break;
                ...
                default:
                    ...
                    // On construit le template
                    $templateHTML = $divInputGlobal.'<div>'.$labelPrincipal.$inputPrincipal.$inputContraintes.$inputValue.$acces.'><p class="p-error"  id="error-' . $this->get("input")["id"] . '"></p></div>';
                    ...
                    if($this->get("input")["confirmationNeeded"] === true){
                        ...
                        // On construit le template
                        $templateHTML .= '<div>'.$labelConfirm.$inputConfirm.$inputContraintes.$inputValue.$acces.'><p class="p-error"  id="error-' . $this->get("input")["id"] . '"></p></div>';
                    }
     ...
     */
    public function verifUnicite($valeur){
