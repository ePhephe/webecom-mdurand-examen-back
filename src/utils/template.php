...
class _template {
     ...
     * @return void
     */
    public function getHtmlHead($title = "", $metadescription = "", $lang = "fr"){
     ...
     * @return mixed Vide si affichage d'une page sinon contenu d'un fragment
     */
    public function getHtmlContent($type, $head = [], $is_nav = true, $is_footer = true){
        ...
        if(!empty($this->name)) {
            ...
            if($type === "pages") {
                // Dans le cas d'une page, un include simple
                include_once("src/templates/" . $type . "/" . $this->name . ".php");
                include_once("src/templates/fragments/message.php");
            }
            else {
                // Dans le cas d'un fragement renvoyé en json, on récupère dans une variable tampon le contenu
                // On démarre la mise en tampon de sortie
                ob_start();
                // On inclut notre fragment
                include_once("src/templates/" . $type . "/" . $this->name . ".php");
                // On récupère le contenu du tampon et on le retourne
                return ob_get_clean();
            }
