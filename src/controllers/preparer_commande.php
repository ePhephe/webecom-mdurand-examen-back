...
/**
 * Classe preparer_commande : classe du controller preparer_commande
 * * Rôle : Passe le statut de la commande en Prête
 */

class preparer_commande extends _controller {
    ...
    // Nom du controller
    protected $name = "PreparerCommande";
    // Liste des objets manipulés par le controller
    protected $objects = ["commande" => ["preparer"]]; // ["objet1" => ["action"1,"action2"...],"objet2" => ["action"1,"action2"...]]
    ...
    public function execute(){
        ...
        if(parent::verifParams()) {
            ...
            if($objCommande->getValue("c_statut") === "AP") {
                // On indique son statut Prête
                $objCommande->set("c_statut","P");
                // On met à jour la date de modification et de préparation
                $dateModification = new DateTime();
                $objCommande->set("c_datetime_modification",$dateModification->format("Y-m-d H:i:s"));
                $objCommande->set("c_datetime_preparation",$dateModification->format("Y-m-d H:i:s"));
                ...
                if($resultat === true) {
                    ...
                    $this->makeRetour(true,"succes","La commande a été passée prête avec succès !");
                }
                else {
                    //On met en forme une erreur si les paramètres ne sont pas présents
                    $this->makeRetour(false,"error","La commande n'a pas pu être passée à prête !");
                }
            }
            else {
                //On met en forme une erreur si les paramètres ne sont pas présents
